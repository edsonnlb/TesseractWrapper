 ...
 * Author:      Ray Smith  : mod by Edson Lemus
...
bool TessBaseAPI::ProcessPages(const char* filename,
  ...
  bool success = ProcessPages(fp, retry_config, timeout_millisec, text_out);
  fclose(fp);

  return success;
}

bool TessBaseAPI::ProcessPages(FILE* fp,
                               const char* retry_config, int timeout_millisec,
                               STRING* text_out) {
  
  if (tesseract_->tessedit_create_hocr)
	  SetInputName("none"); // quick fix to get hocr working
  ...
  //fclose(fp); // why close if it's gonna be used again?
  ...
  Pix *pix;
  if (npages > 0) {
    for (; page < npages && (pix = pixReadStreamTiff(fp, page)) != NULL;
         ++page) {
      if (page > 0)
        tprintf(_("Page %d\n"), page);
      ...
      success &= ProcessPage(pix, page, retry_config,
    ...
    if (pix != NULL) {
      success &= ProcessPage(pix, 0, retry_config,
                             timeout_millisec, text_out);
      pixDestroy(&pix);
    } else {
		// Not used for this dll
...
// for interactively debugging a bad page.
// The text is returned in text_out. Returns false on error.
bool TessBaseAPI::ProcessPage(Pix* pix, int page_index,
                              const char* retry_config, int timeout_millisec,
                              STRING* text_out) {

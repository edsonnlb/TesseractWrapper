/**
Copyright 2011, Cong Nguyen : mod by Edson Lemus
...
/*String* TesseractProcessor::Apply(FILE* fp)
{
	if (_apiInstance == null)
		return null;
	...
	STRING text_out;
	bool succed = api->ProcessPages(fp, null, 0, &text_out);
...
String* TesseractProcessor::Apply(System::Drawing::Image* image, bool hocr)
{
	...
	try
	{
		...
		TessBaseAPI* api = (TessBaseAPI*)_apiInstance.ToPointer();
		result = this->Process(api, pix, hocr);
	}
	catch (System::Exception* exp)
	{
		throw exp;
...
{
	TessBaseAPI* api = (TessBaseAPI*)_apiInstance.ToPointer();
	return this->Process(api, pix, false);
}

String* TesseractProcessor::Process(TessBaseAPI* api, Pix* pix, bool hocr)
{
	...
	api->SetImage(pix);
	//bool succed = api->Recognize(monitor) >= 0;  // this is done twice!!!
	
	char* text = NULL;
	
	if (hocr){
		api->SetInputName("none"); // needs to be called to get hocr working
